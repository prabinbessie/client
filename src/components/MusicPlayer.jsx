...
const MusicPlayer = () => {
  ...
  const nextTrack = () => {
    if (song === allSongs.length - 1) {
      // Go back to the first song if the current song is the last one
      dispatch({
  ...
  const previousTrack = () => {
    if (song === 0) {
      // If the current song is the first one, go to the last song
      dispatch({
        type: actionType.SET_SONG,
  ...
  useEffect(() => {
    if (song >= allSongs.length) {
      // Reset to the first song if the current index is out of bounds
      dispatch({
        type: actionType.SET_SONG,
        song: 0,
      });
